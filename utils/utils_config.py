"""Configuration loader for the bot.

Supports loading from .env file (preferred) with XML fallback for backwards compatibility.
"""
import os
import json
import logging
from dataclasses import dataclass, field
from typing import List, Dict, Set, Optional

# Try to load dotenv (optional dependency)
try:
    from dotenv import load_dotenv  # type: ignore[import-unresolved]
    load_dotenv()
    DOTENV_AVAILABLE = True
except ImportError:
    DOTENV_AVAILABLE = False

from aiogram import Bot, Dispatcher
from aiogram.enums import ContentType
from aiogram.client.default import DefaultBotProperties
from aiogram.enums import ParseMode

# Initialize logger
LOGGER = logging.getLogger(__name__)


@dataclass
class BotConfig:
    """Container for all bot configuration to avoid global statements."""
    # Channel/Group IDs
    CHANNEL_IDS: List[int] = field(default_factory=list)
    CHANNEL_NAMES: List[str] = field(default_factory=list)
    CHANNEL_DICT: Dict[int, str] = field(default_factory=dict)
    
    # Admin group settings
    ADMIN_GROUP_ID: Optional[int] = None
    ADMIN_AUTOREPORTS: Optional[int] = None
    ADMIN_AUTOBAN: Optional[int] = None
    ADMIN_MANBAN: Optional[int] = None
    ADMIN_SUSPICIOUS: Optional[int] = None
    ADMIN_ORDERS: Optional[int] = None
    ADMIN_USER_ID: Optional[int] = None
    SUPERADMIN_GROUP_ID: Optional[int] = None
    
    # Technolog group settings
    TECHNOLOG_GROUP_ID: Optional[int] = None
    TECHNO_LOGGING: Optional[int] = None
    TECHNO_ORIGINALS: Optional[int] = None
    TECHNO_UNHANDLED: Optional[int] = None
    TECHNO_RESTART: Optional[int] = None
    TECHNO_IN: Optional[int] = None
    TECHNO_OUT: Optional[int] = None
    TECHNO_NAMES: Optional[int] = None
    TECHNO_ADMIN: Optional[int] = None
    
    # Bot info
    BOT_NAME: Optional[str] = None
    BOT_USERID: Optional[int] = None
    API_TOKEN: Optional[str] = None
    
    # Group info
    LOG_GROUP: Optional[str] = None
    LOG_GROUP_NAME: Optional[str] = None
    TECHNOLOG_GROUP: Optional[str] = None
    TECHNOLOG_GROUP_NAME: Optional[str] = None
    
    # Spam settings
    SPAM_TRIGGERS: List[str] = field(default_factory=list)
    ALLOWED_FORWARD_CHANNELS: List[Dict] = field(default_factory=list)
    ALLOWED_FORWARD_CHANNEL_IDS: Set[int] = field(default_factory=set)
    
    # Content and updates
    ALLOWED_CONTENT_TYPES: List[ContentType] = field(default_factory=list)
    ALLOWED_UPDATES: List[str] = field(default_factory=list)
    
    # Bot and dispatcher
    BOT: Optional[Bot] = None
    DP: Optional[Dispatcher] = None
    
    # Constants
    MAX_TELEGRAM_MESSAGE_LENGTH: int = 4096
    TELEGRAM_CHANNEL_BOT_ID: int = 136817688
    TELEGRAM_ANONYMOUS_ADMIN_ID: int = 777000  # When admin posts as channel
    P2P_SERVER_URL: str = "http://localhost:8081"
    
    # Established user detection settings
    ESTABLISHED_USER_MIN_MESSAGES: int = 10
    ESTABLISHED_USER_FIRST_MSG_DAYS: int = 90
    HIGH_USER_ID_THRESHOLD: int = 8_400_000_000


# Single config instance - modify attributes, no global keyword needed
config = BotConfig()


def _get_env_or_none(key: str) -> Optional[str]:
    """Get environment variable or return None."""
    value = os.getenv(key)
    return value if value else None


def _get_env_int(key: str, default: Optional[int] = None) -> Optional[int]:
    """Get environment variable as integer."""
    value = os.getenv(key)
    if value:
        try:
            return int(value)
        except ValueError:
            LOGGER.warning("Invalid integer value for %s: %s", key, value)
    return default


def _get_env_list(key: str, separator: str = ",") -> List[str]:
    """Get environment variable as list of strings."""
    value = os.getenv(key)
    if value:
        return [item.strip() for item in value.split(separator) if item.strip()]
    return []


def _get_env_int_list(key: str, separator: str = ",") -> List[int]:
    """Get environment variable as list of integers."""
    value = os.getenv(key)
    if value:
        result = []
        for item in value.split(separator):
            item = item.strip()
            if item:
                try:
                    result.append(int(item))
                except ValueError:
                    LOGGER.warning("Invalid integer in list for %s: %s", key, item)
        return result
    return []


def _get_env_json(key: str, default: Optional[list] = None) -> list:
    """Get environment variable as JSON-parsed list."""
    value = os.getenv(key)
    if value:
        try:
            return json.loads(value)
        except json.JSONDecodeError as e:
            LOGGER.warning("Invalid JSON for %s: %s", key, e)
    return default if default is not None else []


def _get_allowed_content_types() -> List[ContentType]:
    """Get the list of allowed content types."""
    return [
        ContentType.TEXT,
        ContentType.AUDIO,
        ContentType.DOCUMENT,
        ContentType.GAME,
        ContentType.PHOTO,
        ContentType.STICKER,
        ContentType.VIDEO,
        ContentType.VIDEO_NOTE,
        ContentType.VOICE,
        ContentType.CONTACT,
        ContentType.LOCATION,
        ContentType.VENUE,
        ContentType.POLL,
        ContentType.DICE,
        ContentType.INVOICE,
        ContentType.SUCCESSFUL_PAYMENT,
        ContentType.CONNECTED_WEBSITE,
        ContentType.MIGRATE_TO_CHAT_ID,
        ContentType.MIGRATE_FROM_CHAT_ID,
    ]


def load_from_env() -> bool:
    """Load configuration from .env file.
    
    Returns:
        True if loaded successfully, False otherwise.
    """
    # Check if BOT_TOKEN is set (required for .env mode)
    config.API_TOKEN = _get_env_or_none("BOT_TOKEN")
    if not config.API_TOKEN:
        return False

    LOGGER.info("Loading configuration from .env file")

    # Bot credentials
    config.BOT_NAME = _get_env_or_none("BOT_NAME") or "unknown_bot"
    config.BOT_USERID = int(config.API_TOKEN.split(":")[0])

    # Admin settings
    config.ADMIN_USER_ID = _get_env_int("ADMIN_USER_ID")
    config.SUPERADMIN_GROUP_ID = _get_env_int("SUPERADMIN_GROUP_ID")

    # Admin group
    config.ADMIN_GROUP_ID = _get_env_int("ADMIN_GROUP_ID")
    config.LOG_GROUP = str(config.ADMIN_GROUP_ID) if config.ADMIN_GROUP_ID else None
    config.LOG_GROUP_NAME = _get_env_or_none("ADMIN_GROUP_NAME") or "Admin Group"

    # Technolog group
    config.TECHNOLOG_GROUP_ID = _get_env_int("TECHNOLOG_GROUP_ID")
    config.TECHNOLOG_GROUP = str(config.TECHNOLOG_GROUP_ID) if config.TECHNOLOG_GROUP_ID else None
    config.TECHNOLOG_GROUP_NAME = _get_env_or_none("TECHNOLOG_GROUP_NAME") or "Technolog Group"

    # Admin group thread IDs
    config.ADMIN_AUTOREPORTS = _get_env_int("ADMIN_AUTOREPORTS", 1)
    config.ADMIN_AUTOBAN = _get_env_int("ADMIN_AUTOBAN", 1)
    config.ADMIN_MANBAN = _get_env_int("ADMIN_MANBAN", 1)
    config.ADMIN_SUSPICIOUS = _get_env_int("ADMIN_SUSPICIOUS", 1)
    config.ADMIN_ORDERS = _get_env_int("ADMIN_ORDERS", 1)

    # Technolog group thread IDs
    config.TECHNO_LOGGING = _get_env_int("TECHNO_LOGGING", 1)
    config.TECHNO_ORIGINALS = _get_env_int("TECHNO_ORIGINALS", 1)
    config.TECHNO_UNHANDLED = _get_env_int("TECHNO_UNHANDLED", 1)
    config.TECHNO_RESTART = _get_env_int("TECHNO_RESTART", 1)
    config.TECHNO_IN = _get_env_int("TECHNO_IN", 1)
    config.TECHNO_OUT = _get_env_int("TECHNO_OUT", 1)
    config.TECHNO_NAMES = _get_env_int("TECHNO_NAMES", 1)
    config.TECHNO_ADMIN = _get_env_int("TECHNO_ADMIN", 1)

    # Spam triggers
    config.SPAM_TRIGGERS = _get_env_list("SPAM_TRIGGERS")
    if not config.SPAM_TRIGGERS:
        config.SPAM_TRIGGERS = ["url", "email", "phone_number", "hashtag", "mention", 
                                "text_link", "mention_name", "cashtag", "bot_command"]

    # Monitored groups
    config.CHANNEL_IDS = _get_env_int_list("MONITORED_GROUPS")
    config.CHANNEL_NAMES = _get_env_list("MONITORED_GROUP_NAMES")
    
    # Build channel dict
    config.CHANNEL_DICT = {}
    for i, channel_id in enumerate(config.CHANNEL_IDS):
        channel_name = config.CHANNEL_NAMES[i] if i < len(config.CHANNEL_NAMES) else f"Group_{channel_id}"
        config.CHANNEL_DICT[channel_id] = channel_name

    # Allowed forward channels
    config.ALLOWED_FORWARD_CHANNELS = _get_env_json("ALLOWED_FORWARD_CHANNELS", [])
    config.ALLOWED_FORWARD_CHANNEL_IDS = {d["id"] for d in config.ALLOWED_FORWARD_CHANNELS if "id" in d}

    # P2P server
    config.P2P_SERVER_URL = _get_env_or_none("P2P_SERVER_URL") or "http://localhost:8081"

    # Established user detection settings
    config.ESTABLISHED_USER_MIN_MESSAGES = _get_env_int("ESTABLISHED_USER_MIN_MESSAGES", 10) or 10
    config.ESTABLISHED_USER_FIRST_MSG_DAYS = _get_env_int("ESTABLISHED_USER_FIRST_MSG_DAYS", 90) or 90

    # High user ID threshold for new accounts
    config.HIGH_USER_ID_THRESHOLD = _get_env_int("HIGH_USER_ID_THRESHOLD", 8_400_000_000) or 8_400_000_000

    # Content types
    config.ALLOWED_CONTENT_TYPES = _get_allowed_content_types()

    # Initialize bot and dispatcher
    config.BOT = Bot(
        token=config.API_TOKEN,
        default=DefaultBotProperties(parse_mode=ParseMode.HTML)
    )
    config.DP = Dispatcher()
    config.DP["forwarded_reports_states"] = {}
    config.ALLOWED_UPDATES = ["message", "chat_member", "callback_query"]

    config.TELEGRAM_CHANNEL_BOT_ID = 136817688
    config.TELEGRAM_ANONYMOUS_ADMIN_ID = 777000  # When admin posts as channel

    return True


def load_config():
    """Load configuration from .env file."""
    if load_from_env():
        LOGGER.info("Configuration loaded from .env file")
        return
    
    LOGGER.error("Failed to load configuration from .env file! Make sure .env exists with all required variables.")


# Load configuration when the module is imported
load_config()

# Backwards-compatible module-level exports
# These reference the config instance attributes for code that imports them directly
CHANNEL_IDS = config.CHANNEL_IDS
ADMIN_AUTOREPORTS = config.ADMIN_AUTOREPORTS
TECHNO_LOGGING = config.TECHNO_LOGGING
TECHNO_ORIGINALS = config.TECHNO_ORIGINALS
TECHNO_UNHANDLED = config.TECHNO_UNHANDLED
ADMIN_AUTOBAN = config.ADMIN_AUTOBAN
ADMIN_MANBAN = config.ADMIN_MANBAN
TECHNO_RESTART = config.TECHNO_RESTART
TECHNO_IN = config.TECHNO_IN
TECHNO_OUT = config.TECHNO_OUT
ADMIN_USER_ID = config.ADMIN_USER_ID
TECHNO_NAMES = config.TECHNO_NAMES
CHANNEL_NAMES = config.CHANNEL_NAMES
SPAM_TRIGGERS = config.SPAM_TRIGGERS
ADMIN_SUSPICIOUS = config.ADMIN_SUSPICIOUS
TECHNO_ADMIN = config.TECHNO_ADMIN
ADMIN_ORDERS = config.ADMIN_ORDERS
ALLOWED_FORWARD_CHANNELS = config.ALLOWED_FORWARD_CHANNELS
ADMIN_GROUP_ID = config.ADMIN_GROUP_ID
TECHNOLOG_GROUP_ID = config.TECHNOLOG_GROUP_ID
SUPERADMIN_GROUP_ID = config.SUPERADMIN_GROUP_ID
ALLOWED_FORWARD_CHANNEL_IDS = config.ALLOWED_FORWARD_CHANNEL_IDS
MAX_TELEGRAM_MESSAGE_LENGTH = config.MAX_TELEGRAM_MESSAGE_LENGTH
BOT_NAME = config.BOT_NAME
BOT_USERID = config.BOT_USERID
LOG_GROUP = config.LOG_GROUP
LOG_GROUP_NAME = config.LOG_GROUP_NAME
TECHNOLOG_GROUP = config.TECHNOLOG_GROUP
TECHNOLOG_GROUP_NAME = config.TECHNOLOG_GROUP_NAME
DP = config.DP
BOT = config.BOT
ALLOWED_UPDATES = config.ALLOWED_UPDATES
CHANNEL_DICT = config.CHANNEL_DICT
ALLOWED_CONTENT_TYPES = config.ALLOWED_CONTENT_TYPES
API_TOKEN = config.API_TOKEN
TELEGRAM_CHANNEL_BOT_ID = config.TELEGRAM_CHANNEL_BOT_ID
TELEGRAM_ANONYMOUS_ADMIN_ID = config.TELEGRAM_ANONYMOUS_ADMIN_ID
P2P_SERVER_URL = config.P2P_SERVER_URL
ESTABLISHED_USER_MIN_MESSAGES = config.ESTABLISHED_USER_MIN_MESSAGES
ESTABLISHED_USER_FIRST_MSG_DAYS = config.ESTABLISHED_USER_FIRST_MSG_DAYS
HIGH_USER_ID_THRESHOLD = config.HIGH_USER_ID_THRESHOLD
